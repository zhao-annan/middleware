...
import (
	...
	"github.com/zhao-annan/middleware/proto/middleware"
	"k8s.io/api/apps/v1"
...
func (u *MiddlewareDataService) UpdateToK8s(info *middleware.MiddlewareInfo) error {
	...
	if _, err := u.K8sClientSet.AppsV1().StatefulSets(info.MiddleNamespace).Get(context.TODO(), info.MiddleName, v12.GetOptions{}); err != nil {
		...
		if _, err = u.K8sClientSet.AppsV1().StatefulSets(info.MiddleNamespace).
			Update(context.TODO(), statefulSet, v12.UpdateOptions{}); err != nil {
			common.Error(err)
			return err
		}
		common.Info("中间件更新成功" + info.MiddleName + "更新成功")
...
func (u *MiddlewareDataService) DeleteFromK8s(middleware *model.Middleware) (err error) {
	...
	if err := u.K8sClientSet.AppsV1().StatefulSets(middleware.MiddleNamespace).
		Delete(context.TODO(), middleware.MiddleName, v12.DeleteOptions{}); err != nil {
		common.Error(err)
...
func (u *MiddlewareDataService) CreateToK8s(info *middleware.MiddlewareInfo) error {
	...
	if _, err := u.K8sClientSet.AppsV1().StatefulSets(info.MiddleNamespace).Get(context.TODO(),
		info.MiddleName, v12.GetOptions{}); err != nil {

		//如果没有获取到

		if _, err = u.K8sClientSet.AppsV1().StatefulSets(info.MiddleNamespace).Create(context.TODO(),

			statefulSet, v12.CreateOptions{}); err != nil {
		...
		return errors.New("中间件" + info.MiddleName + "已经存在，创建失败")
		...
		pvc := &v13.PersistentVolumeClaim{
			...
			ObjectMeta: v12.ObjectMeta{
				...
				Annotations: map[string]string{
					"pv.kuberneters.io/bound-by-controller":          "yes",
					"volume.beta.kuberneters.io/storage-provisioner": "rbd.csi.ceph.com",
					"Cap": "cap 老师课程创建！",
				},
			},
...
func (u *MiddlewareDataService) getPvcResource(size float32) (source v13.ResourceRequirements) {
	...
	source.Requests = v13.ResourceList{
		"storage": resource.MustParse(strconv.FormatFloat(float64(size), 'f',
			6, 64) + "Gi"),
	}
...
//获取中间件需要的资源
func (u *MiddlewareDataService) getResources(info *middleware.MiddlewareInfo) (
	source v13.ResourceRequirements) {
	//最大能够使用的资源
	source.Limits = v13.ResourceList{
		"cpu": resource.MustParse(strconv.FormatFloat(float64(info.MiddleCpu),
			'f', 6, 64)),
		"memory": resource.MustParse(strconv.FormatFloat(float64(info.MiddleMemory),
			'f', 6, 64)),
	}
	//最小请求资源

	source.Requests = v13.ResourceList{"cpu": resource.MustParse(strconv.FormatFloat(float64(info.MiddleCpu),
		'f', 6, 64)),
		"memory": resource.MustParse(strconv.FormatFloat(float64(info.MiddleMemory),
			'f', 6, 64)),
	}
